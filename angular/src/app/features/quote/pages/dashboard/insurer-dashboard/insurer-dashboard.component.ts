...
import { IUser, OPTIONS_ROLES } from 'src/app/features/admin/user/user.model';
...
@Component({
    ...
    styleUrls: ['./insurer-dashboard.component.scss', './../dashboard.component.scss']
})
export class InsurerDashboardComponent implements OnInit {
    ...
    dashboardData: InsurerDashboardResponse
    ...
    constructor(
        ...
        accountService.currentUser$.subscribe({
            next: (user: IUser) => {
                ...
                //     let passwordExpiresAt = new Date(user.passwordExpiresAt)
                //     if (passwordExpiresAt.getTime() - this.dateToday.getTime() < 24 * 60 * 60 * 1000) {
    ...
    handleBrokerTatOption($event) {
        ...
        console.log(this.selectedAverageBrokerTat)
    }
    ...
    prepareDashboard({ startDate, endDate }: BrokerDashboardRequest) {
        startDate.setHours(5, 30);
        endDate.setHours(29, 29)
        this.quoteService.insurerDashboard({
            startDate: startDate,
            endDate: endDate,
        }).subscribe({
            next: (dto: IOneResponseDto<InsurerDashboardResponse>) => {
                console.log(dto.data.entity)
                this.dashboardData = dto.data.entity
                this.optionsForTat = this.dashboardData.averageInsurerTat

                // if(this.partner.partnerType == AllowedPartnerTypes.broker){
                //     this.optionsForTat =  this.dashboardData.averageBrokerTat
                // }else if(this.partner.partnerType == AllowedPartnerTypes.insurer){
                //     this.optionsForTat =  this.dashboardData.averageInsurerTat
                // }

                this.selectedAverageBrokerTat = this.optionsForTat[0]
                ...
                const remainingSeconds = remainingMinutes % 60;
                this.roundedRemainingSeconds = Math.round(remainingSeconds);
                // this.selectedAverageBrokerTat = this.averageBrokerTat.find((item) => item.label == OPTIONS_BROKER_TAT[0].value)


                // this.averageBrokerTat =

                console.log("Insurer Data", this.dashboardData)

                this.clients = this.dashboardData.clients
                this.stages = this.dashboardData.stages.slice(0, 5)

                //
                console.log(this.selectBrokerDashboardGraph)
                            ...
                            datasets: [
                                {
                                    ...
                                    // data: this.dashboardData.productWiseCountGraph.filter((item) => this.selectedProducts.map((product) => product.label).includes(item.label)).map((item) => item.value),
                                    data: this.dashboardData.productWiseTatGraph.find(product => product.id == this.selectedProduct)?.value?.map((item) => item.value?.toFixed(2) ?? 0),
                ...
                // console.log(this.dashboardData.productWiseCountGraph.filter((item) => this.selectedProducts.map((product) => product.label).includes(item.label)).map((item) => item.label))
    ...
    openAllOpenQuotes() {
        // this.router.navigateByUrl('/backend/quotes?stage=' + JSON.stringify([
    ...
    downloadMisExcel() {
        ...
        for (let i = 0; i < this.dashboardData.clients.length; i++) {
            this.quotesIDForMisExcelDowload.push(this.dashboardData.clients[i].quoteId)
        }

        let fileterdQuotesIDForMisExcelDowload = []

        // To remove the duplicate id
        fileterdQuotesIDForMisExcelDowload = this.quotesIDForMisExcelDowload.filter((id,
            index) => this.quotesIDForMisExcelDowload.indexOf(id) === index);

        console.log(fileterdQuotesIDForMisExcelDowload)

        // this.quoteService.misexcelDownload(fileterdQuotesIDForMisExcelDowload).subscribe((res) => {
        //     console.log("Ids send successfully")
        //     console.log(res);

        //     this.appService.downloadFileFromUrl('Mis Report', res.data.entity.downloadablePath)
        // })

        this.quoteService.misexcelDownloadTake2(fileterdQuotesIDForMisExcelDowload).subscribe((res) => {
            console.log("Ids send successfully")
            console.log(res);
