...
@Component({
    selector: 'app-bsc-electronic-equipments-view-quote-breakup-dialog',
    templateUrl: './bsc-electronic-equipments-view-quote-breakup-dialog.component.html',
    styleUrls: ['./bsc-electronic-equipments-view-quote-breakup-dialog.component.scss']
})
export class BscElectronicEquipmentsViewQuoteBreakupDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.uploadUrl = `${environment.apiUrl}/bsc-electronic-equipments-cover/import-data/${this.config.data.quoteId}`
        // Old_Quote
        ...
        this.bscElectronicEquipmentService.getManyAsLovs({}, lazyLoadEvent).subscribe({
            next: (response: IManyResponseDto<IBscElectronicEquipmentsCover>) => {
                // this.tableRecords = response.data.entities;

                response.data.entities.map((cover: IBscElectronicEquipmentsCover) => {
                    this.bscElectronicEquipmentsCovers[cover.quoteLocationOccupancyId['_id']] = cover;
    ...
    get bulkImportProps(): PFileUploadGetterProps {
        // Old_Quote
        // return this.bscElectronicEquipmentService.getBulkImportProps(this.config.data.quoteId, (dto: IOneResponseDto<IBulkImportResponseDto>) => {

        // New_Quote_Option
        return this.bscElectronicEquipmentService.getBulkImportProps(this.config.data.quoteOption._id, (dto: IOneResponseDto<IBulkImportResponseDto>) => {
    ...
    bulkImportGenerateSample() {
        // Old_Quote
        // this.bscElectronicEquipmentService.bulkImportGenerateSample(this.config.data.quoteId).subscribe({

        // New_Quote_Option
        this.bscElectronicEquipmentService.bulkImportGenerateSample(this.config.data.quoteOption._id).subscribe({
