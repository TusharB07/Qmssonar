...
@Component({
  selector: 'app-cost-containment-tab',
  templateUrl: './cost-containment-tab.component.html',
  styleUrls: ['./cost-containment-tab.component.scss']

})
export class CostContainmentTabComponent implements OnInit {
  ...
  constructor(private accountService: AccountService, private quoteService: QuoteService, private gmcQuoteTemplateService: QoutegmctemplateserviceService, private messageService: MessageService) {
    ...
    this.currentSelectedTemplate = this.quoteService.currentSelectedOptions$.subscribe({
      next: (template) => {
        ...
        this.tabsData = template.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COSTCONTAINMENT)[0]
        ...
        this.tabsData.gmcSubTab.forEach(element => {
          element.gmcLabelForSubTab.forEach(elementSubTab => {
            elementSubTab.gmcQuestionAnswers.forEach(elementQues => {
              if (elementQues?.question?.toLowerCase().trim() == 'ailment limit') {              
                if (elementQues?.selectedAnswer == 'Applicable') {
  ...
  onRadioChange(event: Event, itemquestion: any) {
    const selectedValue = (event.target as HTMLInputElement).value;
    console.log('Selected Value:', selectedValue);
    console.log('Associated Question:', itemquestion);

    if (itemquestion.question == 'Ailment Limit') {
      if(itemquestion.selectedAnswer == 'Applicable'){
        this.isModalVisible = true; // Open the modal
      }
      else{
  ...
  onDDLChange(event: any, itemquestion: any, gmcQuestionAnswers: any) {
    ...
    if (itemquestion == 'Ailment Limit' && this.ddlText == 'Applicable') {
      this.isModalVisible = true; // Open the modal
    }
          ...
          this.quoteService.getAllQuoteOptions(baseQuoteId).subscribe({
            next: (dto: IOneResponseDto<IQuoteGmcTemplate[]>) => {
              ...
              const baseQuoteData = baseOption.gmcTemplateData.find(
                (x) => x.parentTabName === AllowedGMCPARENTabsTypes.COSTCONTAINMENT
              );
              ...
              const updatedGmcTemplateData = this.selectedQuoteTemplate.gmcTemplateData.map((item) => {
                if (item.parentTabName === AllowedGMCPARENTabsTypes.COSTCONTAINMENT) {
              ...
              this.tabsData = this.selectedQuoteTemplate.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COSTCONTAINMENT)[0]
