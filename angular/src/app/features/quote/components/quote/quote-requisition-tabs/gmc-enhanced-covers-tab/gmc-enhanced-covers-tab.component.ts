...
import { BasicDetailsAttachments, CoverageDetails, IEmployeeData, IEmployeesDemoSummary, IGmcDataModel, IQuoteGmcTemplate, IQuoteSlip } from 'src/app/features/admin/quote/quote.model';
...
@Component({
  selector: 'app-gmc-enhanced-covers-tab',
  templateUrl: './gmc-enhanced-covers-tab.component.html',
  styleUrls: ['./gmc-enhanced-covers-tab.component.scss']
})
export class GmcEnhancedCoversTabComponent implements OnInit {
  ddlText: string = ""
  ...
  constructor(private accountService: AccountService, private quoteService: QuoteService, private gmcQuoteTemplateService: QoutegmctemplateserviceService, private messageService: MessageService) {
    ...
    this.currentSelectedTemplate = this.quoteService.currentSelectedOptions$.subscribe({
      next: (template) => {
        ...
        this.tabsData = template.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.ENHANCEDCOVERS)[0]
        this.tabsData.gmcSubTab.forEach(element => {
          element.gmcLabelForSubTab.forEach(elementSubTab => {
            elementSubTab.gmcQuestionAnswers.forEach(elementQues => {
              if (elementQues?.inputControl == 'radiobutton') {
                const selectedAnswer = elementQues?.answer.find((x: any) => x._id === elementQues?.selectedAnswer)[0];
                elementQues.selectedAnswer = selectedAnswer.answer
              }
            });
          });
        });
  ...
  onCheckboxChange(event: any, gmcQuestionAnswers: any[]) {
    gmcQuestionAnswers.forEach(element => {
      if (element.inputControl === 'dropdown') {
        ...
        if (event.checked) {
          element.selectedAnswer = element.answer.some(x => x.answer.toLowerCase() === 'covered') ? "Covered"
            : element.answer.some(x => x.answer.toLowerCase()=== 'applicable') ? "Applicable"
              : null;
        } else {
          element.selectedAnswer = element.answer.some(x => x.answer.toLowerCase() === 'covered') ? "Not covered"
            : element.answer.some(x => x.answer.toLowerCase() === 'applicable') ? "Not applicable"
              : null;
        }
      }
      ...
      else if (element.inputControl === 'checkbox') {
        element.selectedAnswer = event.checked
      }
    });
  }
          ...
          this.quoteService.getAllQuoteOptions(baseQuoteId).subscribe({
            next: (dto: IOneResponseDto<IQuoteGmcTemplate[]>) => {
              ...
              const baseQuoteData = baseOption.gmcTemplateData.find(
                (x) => x.parentTabName === AllowedGMCPARENTabsTypes.ENHANCEDCOVERS
              );
              ...
              const updatedGmcTemplateData = this.selectedQuoteTemplate.gmcTemplateData.map((item) => {
                if (item.parentTabName === AllowedGMCPARENTabsTypes.ENHANCEDCOVERS) {
              ...
              this.tabsData = this.selectedQuoteTemplate.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.ENHANCEDCOVERS)[0]
