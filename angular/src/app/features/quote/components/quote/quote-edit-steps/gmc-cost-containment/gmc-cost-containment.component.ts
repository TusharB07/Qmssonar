...
@Component({
  selector: 'app-gmc-cost-containment',
  templateUrl: './gmc-cost-containment.component.html',
  styleUrls: ['./gmc-cost-containment.component.scss']
})
export class GmcCostContainmentComponent implements OnInit {
  expanded = false;
    ...
    this.currentSelectedTemplate = this.quoteService.currentQuoteOptions$.subscribe({
      next: (template) => {
        ...
        this.selectedQuoteTemplate.forEach(element => {
          ...
          element.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COSTCONTAINMENT).forEach(tempdata => {
            //Temp ID
        ...
        // this.selectedQuoteTemplate.forEach(element => {
        //   const dd = element.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COSTCONTAINMENT)[0]
