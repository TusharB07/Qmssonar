...
@Component({
  selector: 'app-gmc-enhanced-covers',
  templateUrl: './gmc-enhanced-covers.component.html',
  styleUrls: ['./gmc-enhanced-covers.component.scss']
})
export class GmcEnhancedCoversComponent implements OnInit {
  expanded = false;
    ...
    this.currentSelectedTemplate = this.quoteService.currentQuoteOptions$.subscribe({
      next: (template) => {
        ...
        this.selectedQuoteTemplate.forEach(element => {
          ...
          element.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.ENHANCEDCOVERS).forEach(tempdata => {
            //Temp ID
            const tempId = tempdata._id
            if(tempdata.gmcSubTab.length>0){             
              if (optionIndex == 1) {
        ...
        // this.selectedQuoteTemplate.forEach(element => {
        //   const dd = element.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COVERAGES)[0]
