...
@Component({
  selector: 'app-gmc-coverages',
  templateUrl: './gmc-coverages.component.html',
  styleUrls: ['./gmc-coverages.component.scss']
})
export class GmcCoveragesComponent implements OnInit {
  expanded = false;
    ...
    this.currentSelectedTemplate = this.quoteService.currentQuoteOptions$.subscribe({
      next: (template) => {
        ...
        this.selectedQuoteTemplate.forEach(element => {
          ...
          element.gmcTemplateData.filter(x => x.parentTabName == AllowedGMCPARENTabsTypes.COVERAGES).forEach(tempdata => {
            //Temp ID
