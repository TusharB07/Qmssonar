...
import { GmcGradedSI, IQuoteGmcTemplate, IQuoteSlip, QuoteGmcTemplate } from 'src/app/features/admin/quote/quote.model';
import { QuoteService } from 'src/app/features/admin/quote/quote.service';
import { SumInsuredService } from 'src/app/features/admin/sumInsured/suminsured.service';
import { ToWords } from 'to-words';

@Component({
  selector: 'app-gmc-graded-si-dialog',
  templateUrl: './gmc-graded-si-dialog.component.html',
  styleUrls: ['./gmc-graded-si-dialog.component.scss']
})
export class GmcGradedSiDialogComponent implements OnInit {
  ...
  constructor(private messageService: MessageService, public config: DynamicDialogConfig, private quoteService: QuoteService, private sumInsuredService: SumInsuredService,
      ...
      next: (dto: IOneResponseDto<IQuoteGmcTemplate[]>) => {
        ...
        if (this.quoteGmcOptionsLst.length > 0) {
          ...
          if (this.selectedQuoteTemplate != undefined) {
            optioname = this.selectedQuoteTemplate.optionName;
          }
          this.quoteGmcOptions = this.quoteGmcOptionsLst.filter(x => x.optionName == optioname)[0];
          this.gmcGradedSILst = this.quoteGmcOptions.gmcGradedSILst.sort((a, b) => (a.grade < b.grade ? -1 : 1));
          console.log(this.gmcGradedSILst)
          ...
          if (this.gmcGradedSILst.length == 0) {
            ...
            this.gmcGradedSILst.sort((a, b) => (a.grade < b.grade ? -1 : 1));
          }
          else {
    ...
    this.gmcQuoteTemplateService.updateArray(this.selectedQuoteTemplate._id, updatePayload).subscribe({
      next: partner => {
        ...
        this.messageService.add({
          severity: "success",
          summary: "Successful",
          detail: `Graded SI saved successfully!`,
          life: 3000
        });
      },
      error: error => {
