...
@Component({
  ...
  templateUrl: './subjectivity-list.component.html',
  styleUrls: ['./subjectivity-list.component.scss']
})
export class SubjectivityListComponent {

  cols: any[];

  recordSingularName = "Subjectivity";
  recordPluralName = "Subjectivitys";
  modulePath: string = "/backend/admin/subjectivity";
  /** Represents the data being displayed currently */
  records: ISubjectivity[];
  /** Represents the user record being deleted. */
  selectedRecord: ISubjectivity | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: ISubjectivity[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.subjectivityService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.subjectivityService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.subjectivityService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
