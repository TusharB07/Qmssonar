...
@Component({
  selector: 'app-wc-desc-of-employee-list',
  templateUrl: './wc-desc-of-employee-list.component.html',
  styleUrls: ['./wc-desc-of-employee-list.component.scss']
})
export class WCDescriptionOfEmployeeListComponent implements OnInit, OnDestroy {
  ...
  records: IWCDescriptionOfEmployee[];
  /** Represents desc of emp record being deleted. */
  selectedRecord: IWCDescriptionOfEmployee | null = null;
  /** Represents desc of emp records being deleted. */
  selectedRecords: IWCDescriptionOfEmployee[] = [];
  ...
  ngOnInit(): void {
    ...
    this.action = [
      { name: 'Import', code: 'IM', rate: 'wc_DescriptionOfEmployee' },
      { name: 'Export', code: 'EX', rate: 'wc_DescriptionOfEmployee' }
    ];

    this.filterData = this.wcDescriptionOfEmployeeService.getFilterData();
  }
  ...
  loadRecords(event: LazyLoadEvent, name?: string) {
    let filterValue = this.wcDescriptionOfEmployeeService.getFilterValueExist();
    if (name) {
      let filtermetadata = {}
      let values = [];
    ...
    this.wcDescriptionOfEmployeeService.getMany(event).subscribe({
      next: records => {
        console.log(records.data.entities.map((i: IWCDescriptionOfEmployee) => [i.description, i.productId]));
        this.filterData = null;
        this.wcDescriptionOfEmployeeService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.wcDescriptionOfEmployeeService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.wcDescriptionOfEmployeeService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  ngOnDestroy(){
    this.wcDescriptionOfEmployeeService.setFilterValueExist(null);
  }
