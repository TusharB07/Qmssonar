...
@Component({
  selector: "app-addon-cover-sector-list",
  templateUrl: "./addon-cover-sector-list.component.html",
  styleUrls: ["./addon-cover-sector-list.component.scss"]
})
export class AddonCoverSectorListComponent implements OnInit {
  @Input() addOnCoverId: string = "";
  ...
  selectedRecord: IAddOnCoverSector | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IAddOnCoverSector[] = [];
  ...
  ngOnInit(): void {
    this.cols = [
      { field: "_id", header: "Id" },
      { field: "addOnCoverId", header: "Addon Cover" },
      { field: "stsectorIdte", header: "Sector" },
      { field: "isApplicable", header: "Is Applicable" },
      { field: "status", header: "Status" },
    ];

    if(this.addOnCoverId) {
      this.modulePath = `/backend/admin/addon-covers/${this.addOnCoverId}/addon-cover-sector`;
    }
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    if (this.addOnCoverId != "") {
      // @ts-ignore
      event.filters["addOnCoverId"] = [
        {
          value: this.addOnCoverId,
          matchMode: "equals",
          operator: "and"
        }
      ];
