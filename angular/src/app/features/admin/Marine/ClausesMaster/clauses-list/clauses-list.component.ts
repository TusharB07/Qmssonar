...
export class ClausesListComponent implements OnInit, OnDestroy {
  ...
  selectedRecord: IClauses | null = null;
  /** Represents the SI records being deleted. */
  selectedRecords: IClauses[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.clausesService.getFilterData();
  }

  loadRecords(event: LazyLoadEvent) {
    let filterValue = this.clausesService.getFilterValueExist();
    ...
    this.loading = true;
    this.clausesService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.clausesService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.clausesService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.clausesService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.clausesService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
  ...
  ngOnDestroy(){
    this.clausesService.setFilterValueExist(null);
  }
