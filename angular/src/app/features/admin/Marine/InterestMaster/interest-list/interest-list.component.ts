...
export class InterestListComponent implements OnInit, OnDestroy {
  ...
  selectedRecord: IInterest | null = null;
  /** Represents the SI records being deleted. */
  selectedRecords: IInterest[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.interestService.getFilterData();
  }

  loadRecords(event: LazyLoadEvent) {
    let filterValue = this.interestService.getFilterValueExist();
    ...
    this.loading = true;
    this.interestService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.interestService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.interestService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.interestService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.interestService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
  ...
  ngOnDestroy(){
    this.interestService.setFilterValueExist(null);
  }
