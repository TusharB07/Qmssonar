...
export class PackagingListComponent implements OnInit,OnDestroy {
  ...
  selectedRecord: IPackaging | null = null;
  /** Represents the SI records being deleted. */
  selectedRecords: IPackaging[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.packagingService.getFilterData();
  }

  loadRecords(event: LazyLoadEvent) {
    let filterValue = this.packagingService.getFilterValueExist();
    ...
    this.loading = true;
    this.packagingService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.packagingService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.packagingService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.packagingService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.packagingService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
  ...
  ngOnDestroy(){
    this.packagingService.setFilterValueExist(null);
  }
