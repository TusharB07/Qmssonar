...
@Component({
  selector: 'app-transitType-list',
  templateUrl: './transitType-list.component.html',
  styleUrls: ['./transitType-list.component.scss']
})
export class TransitTypeListComponent implements OnInit, OnDestroy {
  ...
  records: ITransitType[];
  /** Represents the SI record being deleted. */
  selectedRecord: ITransitType | null = null;
  /** Represents the SI records being deleted. */
  selectedRecords: ITransitType[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.transitTypeService.getFilterData();
  }

  loadRecords(event: LazyLoadEvent) {
    let filterValue = this.transitTypeService.getFilterValueExist();
    ...
    this.loading = true;
    this.transitTypeService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.transitTypeService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.transitTypeService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.transitTypeService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.transitTypeService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
  ...
  ngOnDestroy(){
    this.transitTypeService.setFilterValueExist(null);
  }
