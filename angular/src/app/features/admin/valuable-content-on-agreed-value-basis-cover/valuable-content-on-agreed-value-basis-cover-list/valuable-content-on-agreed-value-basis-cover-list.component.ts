...
@Component({
  selector: 'app-valuable-content-on-agreed-value-basis-cover-list',
  templateUrl: './valuable-content-on-agreed-value-basis-cover-list.component.html',
  styleUrls: ['./valuable-content-on-agreed-value-basis-cover-list.component.scss']
})
export class ValuableContentOnAgreedValueBasisCoverListComponent implements OnInit {
  ...
  records: IValuableContentsOnAgreedValue[];
  /** Represents the user record being deleted. */
  selectedRecord: IValuableContentsOnAgreedValue | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IValuableContentsOnAgreedValue[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.valuableContentOnAgreedValueBasisCoverService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.valuableContentOnAgreedValueBasisCoverService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.valuableContentOnAgreedValueBasisCoverService.deleteMany(userIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
