...
@Component({
  selector: 'app-icrm-contact-list',
  templateUrl: './icrm-contact-list.component.html',
  styleUrls: ['./icrm-contact-list.component.scss']
})
export class IcrmContactListComponent implements OnInit {

  cols: any[];

  recordSingularName = "ICRM Contact";
  recordPluralName = "ICRM Contacts";
  modulePath: string = "/backend/admin/icrm-contact";
  /** Represents the data being displayed currently */
  records: IICRMContact[];
  /** Represents the user record being deleted. */
  selectedRecord: IICRMContact | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IICRMContact[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.icrmContactService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.icrmContactService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.icrmContactService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
