...
import { LossOfRentCoverService } from '../loss-of-rent-cover.service';
...
@Component({
  selector: 'app-loss-of-rent-cover-list',
  templateUrl: './loss-of-rent-cover-list.component.html',
  styleUrls: ['./loss-of-rent-cover-list.component.scss']
})
export class LossOfRentCoverListComponent implements OnInit {

  cols: any[];
  
  recordSingularName = "Loss of Rent Cover";
  recordPluralName = "Loss of Rent Cover";
  modulePath: string = "/backend/admin/loss-of-rent-cover";
  /** Represents the data being displayed currently */
  records: ILossOfRent[];
  /** Represents the user record being deleted. */
  selectedRecord: ILossOfRent | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: ILossOfRent[] = [];
  ...
  constructor(private router: Router,
     ...
     private lossOfRentCoverService: LossOfRentCoverService,
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.lossOfRentCoverService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.lossOfRentCoverService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.lossOfRentCoverService.deleteMany(userIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
