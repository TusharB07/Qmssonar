...
import { BscMoneySafeTillService } from '../bsc-money-safe-till.service';
import { IQuoteLocationOccupancy } from '../../quote-location-occupancy/quote-location-occupancy.model';
import { LazyLoadEvent } from 'primeng/api';

@Component({
  selector: 'app-bsc-money-safe-till-form',
  templateUrl: './bsc-money-safe-till-form.component.html',
  styleUrls: ['./bsc-money-safe-till-form.component.scss']
})
export class BscMoneySafeTillFormComponent implements OnInit {
  id: string;
  mode: FormMode = "new";
  recordForm: FormGroup;
  submitted: boolean = false;

  recordSingularName = "BSC Money Safe Till";
  recordPluralName = "BSC Money Safe Till";
  modulePath: string = "/backend/admin/bsc-money-safe-till-cover";
  optionsQuotes: ILov[] = [];
  ...
  constructor(
    ...
    private recordService: BscMoneySafeTillService,
  ...
  ngOnInit(): void {
    ...
    if (this.id !== "new") {
      ...
      this.recordService.get(this.id).subscribe({
        next: (dto: IOneResponseDto<IBscMoneySafeTillCover>) => {
  ...
  createForm(item?: IBscMoneySafeTillCover) {
    ...
    const quoteLocationOccupancy: IQuoteLocationOccupancy = item?.quoteLocationOccupancyId as IQuoteLocationOccupancy;
    this.recordForm = this.formBuilder.group({
      _id: [item?._id],
      locationId: [item?.locationId],
      occupancyId: [item?.occupancyId],
      moneySafe: [item?.moneySafe, [ Validators.min(0)]],
      moneyTillCounter: [item?.moneyTillCounter, [Validators.min(0)]],
      ...
      quoteLocationOccupancy: [quoteLocationOccupancy ? { label: quoteLocationOccupancy.locationName, value: quoteLocationOccupancy._id } : null, [Validators.required]],
