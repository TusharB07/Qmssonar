...
@Component({
  selector: 'app-personal-accident-cover-list',
  templateUrl: './personal-accident-cover-list.component.html',
  styleUrls: ['./personal-accident-cover-list.component.scss']
})
export class PersonalAccidentCoverListComponent implements OnInit {
  ...
  records: IPersonalAccidentCover[];
  /** Represents the user record being deleted. */
  selectedRecord: IPersonalAccidentCover | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IPersonalAccidentCover[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.personalAccidentCoverService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.personalAccidentCoverService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.personalAccidentCoverService.deleteMany(userIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
