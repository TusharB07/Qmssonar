...
@Component({
  selector: 'app-wc-salary-slabs-list',
  templateUrl: './wc-salary-slabs-list.component.html',
  styleUrls: ['./wc-salary-slabs-list.component.scss']
})
export class SalarySlabsListComponent implements OnInit, OnDestroy {
  ...
  records: ISalarySlabs[];
  /** Represents the slabs record being deleted. */
  selectedRecord: ISalarySlabs | null = null;
  /** Represents the slabs records being deleted. */
  selectedRecords: ISalarySlabs[] = [];
  ...
  ngOnInit(): void {
    ...
    this.action = [
      { name: 'Import', code: 'IM', rate: 'wc_SalarySlabs' },
      { name: 'Export', code: 'EX', rate: 'wc_SalarySlabs' }
    ];
    this.filterData = this.salarySlabsService.getFilterData();
  }
  ...
  //   this.loading = true;
  //   this.salarySlabsService.getMany(event).subscribe({
  //     next: records => {
  //       console.log(records);
  ...
  loadRecords(event: LazyLoadEvent, name?: string) {
    let filterValue = this.salarySlabsService.getFilterValueExist();
    if (name) {
      let filtermetadata = {}
      let values = [];
    ...
    this.salarySlabsService.getMany(event).subscribe({
      next: records => {
        console.log(records.data.entities.map((i: ISalarySlabs) => [i.salaryStr, i.productId]));
        this.filterData = null;
        this.salarySlabsService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.salarySlabsService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.salarySlabsService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  ngOnDestroy(){
    this.salarySlabsService.setFilterValueExist(null);
  }
