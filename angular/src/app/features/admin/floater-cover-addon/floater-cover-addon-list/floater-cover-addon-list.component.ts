...
import { FloaterCoverAddonService } from '../floater-cover-addon.service';
  ...
  @Component({
    selector: 'app-floater-cover-addon-list',
    templateUrl: './floater-cover-addon-list.component.html',
    styleUrls: ['./floater-cover-addon-list.component.scss']
  })
  
  export class FloaterCoverAddonListComponent implements OnInit {
    ...
    records: IFloaterCoverAddOn[];
    /** Represents the user record being deleted. */
    selectedRecord: IFloaterCoverAddOn | null = null;
    /** Represents the user records being deleted. */
    selectedRecords: IFloaterCoverAddOn[] = [];
    ...
    constructor(private router: Router, private messageService: MessageService,  private floaterCoverAddonService: FloaterCoverAddonService, private breadcrumbService: AppBreadcrumbService) {
    ...
    loadRecords(event: LazyLoadEvent) {
      ...
      this.loading = true;
      this.floaterCoverAddonService.getMany(event).subscribe({
        next: records => {
          console.log(records);
    ...
    deleteRecord() {
      ...
      this.floaterCoverAddonService.delete(this.selectedRecord._id).subscribe({
        next: res => {
    ...
    deleteSelectedRecords() {
      ...
      this.floaterCoverAddonService.deleteMany(userIds).subscribe({
        next: v => {
          this.loadRecords(DEFAULT_RECORD_FILTER);
          this.messageService.add({
            severity: "success",
