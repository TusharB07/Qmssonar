...
@Component({
  selector: 'app-leadInsurer-list',
  templateUrl: './leadInsurer-list.component.html',
  styleUrls: ['./leadInsurer-list.component.scss']
})
export class LeadInsurersListComponent implements OnInit {
  ...
  records: ILeadInsurers[];
  /** Represents the LI record being deleted. */
  selectedRecord: ILeadInsurers | null = null;
  /** Represents the LI records being deleted. */
  selectedRecords: ILeadInsurers[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.leadInsurersService.getFilterData();
  }
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.leadInsurersService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.leadInsurersService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.leadInsurersService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.leadInsurersService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.leadInsurersService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
