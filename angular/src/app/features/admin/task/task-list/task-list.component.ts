...
export class TaskListComponent implements OnInit, OnDestroy {
  ...
  selectedRecord: ITask | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: ITask[] = [];
    ...
    this.recordService.getManyAsLovs(e.query).subscribe({
      next: data => {
        ...
        if (e.query) {
          ...
          this.optionsParentLovId = data.data.entities.filter(entity => entity.state.toLowerCase().includes(e.query.toLowerCase()))
            .map(entity => ({ label: entity.state, value: entity._id }));
            this.optionsParentLovId.sort((a,b)=> a.label.toLowerCase() > b.label.toLowerCase() ? 1 : -1);
        } else {
          this.optionsParentLovId = data.data.entities.map(entity => ({ label: entity.state, value: entity._id }));
          this.optionsParentLovId.sort((a,b)=> a.label.toLowerCase() > b.label.toLowerCase() ? 1 : -1);
        }
      },
      error: e => { }
