...
@Component({
  selector: 'app-wc-business-type-list',
  templateUrl: './wc-business-type-list.component.html',
  styleUrls: ['./wc-business-type-list.component.scss']
})
export class BusinessTypeListComponent implements OnInit, OnDestroy {
  ...
  records: IBusinessType[];
  /** Represents the business type record being deleted. */
  selectedRecord: IBusinessType | null = null;
  /** Represents the business type records being deleted. */
  selectedRecords: IBusinessType[] = [];
  ...
  ngOnInit(): void {
    ...
    this.action = [
      { name: 'Import', code: 'IM', rate: 'wc_businesstypes' },
      { name: 'Export', code: 'EX', rate: 'wc_businesstypes' }
    ];
    this.filterData = this.businessTypeService.getFilterData();
  }
  ...
  loadRecords(event: LazyLoadEvent, name?: string) {
    let filterValue = this.businessTypeService.getFilterValueExist();
    if (name) {
      let filtermetadata = {}
      let values = [];
    ...
    this.businessTypeService.getMany(event).subscribe({
      next: records => {
        console.log(records.data.entities.map((i: IBusinessType) => [i.businessType, i.productId]));
        this.filterData = null;
        this.businessTypeService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.businessTypeService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.businessTypeService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  ngOnDestroy(){
    this.businessTypeService.setFilterValueExist(null);
  }
