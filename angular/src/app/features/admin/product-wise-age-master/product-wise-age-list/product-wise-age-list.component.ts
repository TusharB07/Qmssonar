...
@Component({
  selector: 'app-product-wise-age-list',
  templateUrl: './product-wise-age-list.component.html',
  styleUrls: ['./product-wise-age-list.component.scss']
})
export class ProduxtWiseAgeListComponent implements OnInit {
  ...
  records: IProductWiseAge[];
  /** Represents the Product wise age record being deleted. */
  selectedRecord: IProductWiseAge | null = null;
  /** Represents the Product wise age records being deleted. */
  selectedRecords: IProductWiseAge[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.productWiseAgeService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.productWiseAgeService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.productWiseAgeService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
