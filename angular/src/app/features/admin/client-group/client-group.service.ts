...
export class ClientGroupService extends CrudService<IClientGroup> {
  constructor(protected http: HttpClient, protected accountService: AccountService) {
    super(`${environment.apiUrl}/client-groups`, http, accountService, { populate: ["cityId", , "pincodeId"] });
  }

  // To List Records from the DB
  getMatchingClientGroups(search: string): Observable<IManyResponseDto<IClientGroup>> {
    return this.http.get<IManyResponseDto<IClientGroup>>(`${this.baseUrl}/match?search=${search}`, {
