...
@Component({
  selector: 'app-annual-turnover-list',
  templateUrl: './annual-turnover-list.component.html',
  styleUrls: ['./annual-turnover-list.component.scss']
})
export class AnnualTurnoverListComponent implements OnInit {
  ...
  records: IAnnualTurnover[];
  /** Represents the annual turnover record being deleted. */
  selectedRecord: IAnnualTurnover | null = null;
  /** Represents the annual turnover records being deleted. */
  selectedRecords: IAnnualTurnover[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.annualTurnoverService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.annualTurnoverService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.annualTurnoverService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
