...
export class ExclusionListComponent implements OnInit {
  ...
  selectedRecord: IExclusion | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IExclusion[] = [];
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.exclusionService.getMany(event).subscribe({
      next: records => {
        console.log(records);
  ...
  deleteRecord() {
    ...
    this.exclusionService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.exclusionService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
