...
@Component({
  selector: 'app-thirdPartyAdministrators-list',
  templateUrl: './thirdPartyAdministrators-list.component.html',
  styleUrls: ['./thirdPartyAdministrators-list.component.scss']
})
export class  ThirdPartyAdministratorsListComponent implements OnInit {
  ...
  records: IThirdPartyAdministrators[];
  /** Represents the LI record being deleted. */
  selectedRecord: IThirdPartyAdministrators | null = null;
  /** Represents the LI records being deleted. */
  selectedRecords: IThirdPartyAdministrators[] = [];
  ...
  ngOnInit(): void {
    this.filterData = this.thirdPartyAdministratorsService.getFilterData();
  }
  ...
  loadRecords(event: LazyLoadEvent) {
    ...
    this.loading = true;
    this.thirdPartyAdministratorsService.getMany(event).subscribe({
      next: records => {
        console.log(records);
        this.filterData = null;
        this.thirdPartyAdministratorsService.setFilterData(event);
  ...
  deleteRecord() {
    ...
    this.thirdPartyAdministratorsService.delete(this.selectedRecord._id).subscribe({
      next: res => {
  ...
  deleteSelectedRecords() {
    ...
    this.thirdPartyAdministratorsService.deleteMany(productIds).subscribe({
      next: v => {
        this.loadRecords(DEFAULT_RECORD_FILTER);
        this.messageService.add({
          severity: "success",
  ...
  clear(table: Table) {
    this.thirdPartyAdministratorsService.setFilterData(undefined);
    this.filterData = null;
    this.selectedRecords = [];
    this.selectAll = false;
    table.clear();
