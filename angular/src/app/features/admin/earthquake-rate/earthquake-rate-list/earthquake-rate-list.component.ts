...
import { AllowedTaskStatus, IEarthquakeRate } from "../earthquake-rate.model";
...
@Component({
  selector: "app-earthquake-rate-list",
  templateUrl: "./earthquake-rate-list.component.html",
  styleUrls: ["./earthquake-rate-list.component.scss"]
})
export class EarthquakeRateListComponent implements OnInit,OnDestroy {
  cols: any[];
  ...
  records: IEarthquakeRate[];
  /** Represents the user record being deleted. */
  selectedRecord: IEarthquakeRate | null = null;
  /** Represents the user records being deleted. */
  selectedRecords: IEarthquakeRate[] = [];
  ...
  loadRecords(event: LazyLoadEvent, name?: string) {
    ...
    this.recordService.getMany(event).subscribe({
      next: records => {
        // console.log(records.data.entities.map((i: IEarthquakeRate) => [i.industryTypeId, i.productId]));
